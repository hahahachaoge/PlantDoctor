...
METADATA_FILE = BASE_DIR / "disease_metadata.json"
...
if METADATA_FILE.exists():
    with METADATA_FILE.open("r", encoding="utf-8") as file_obj:
        metadata_document = json.load(file_obj)
    DISEASE_METADATA = metadata_document.get("entries", {})
    print(f"已加载 {len(DISEASE_METADATA)} 条别名与分布资料")
else:
    print("未找到 disease_metadata.json，将使用默认别名与分布提示")
...
ALIASES = {
    ...
    "Insect_Salurnis_marginella_Guerr": "褐缘蛾蜡蝉",
    ...
    "Insect_cerodonta_denticornis": "麦黄潜叶蝇",
...
def get_info(raw_name):
    metadata = DISEASE_METADATA.get(raw_name, {})
    if raw_name in DISEASE_INFO:
        ...
        return {
            ...
            "alias": metadata.get("alias", "暂无经核实的常用别名"),
            "region": metadata.get("region", "暂无经核实的分布资料"),
        }
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("启动病虫害识别 API")
    print("局域网访问: http://<本机IPv4>:8000（请用 ipconfig 或 ip addr 查询）")
    print("=" * 50)
