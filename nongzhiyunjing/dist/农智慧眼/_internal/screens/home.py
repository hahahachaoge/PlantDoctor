...
class HomeScreen(Screen):
    ...
    def _build_bottom_nav(self):
        ...
        items = [
            ("首页", lambda *_: setattr(self.manager, "current", "home"), True),
            ("社区", lambda *_: setattr(self.manager, "current", "community"), False),
            ("拍照", lambda *_: App.get_running_app().show_capture_menu(), False),
            ("百科", lambda *_: setattr(self.manager, "current", "store"), False),
            ("我的", lambda *_: setattr(self.manager, "current", "mypage"), False),
        ]
        for title, callback, is_active in items:
            item_box = BoxLayout(orientation="vertical", spacing=dp(2),
                                 size_hint=(1, 1))
            if title == "拍照":
                btn_wrap = FloatLayout(size_hint=(1, None), height=dp(42))
                circle_btn = Button(
                    text="+",
                ...
                btn = Button(
                    ...
                    background_normal="", background_down="",
                    background_color=(0, 0, 0, 0),
                    color=GREEN if is_active else (0.55, 0.55, 0.55, 1),
                    **text_style(),
                )
                ...
                if is_active:
                    ...
                    dot.pos_hint = {"center_x": 0.5}
                    with dot.canvas:
                        Color(*GREEN)
                        dot_ellipse = RoundedRectangle(
                            pos=dot.pos, size=dot.size, radius=[dp(3)] * 4)
                    dot.bind(
                        pos=lambda i, e=dot_ellipse, *_: setattr(e, "pos", i.pos),
                        size=lambda i, e=dot_ellipse, *_: setattr(e, "size", i.size),
                    )
                    item_box.add_widget(dot)

            nav.add_widget(item_box)
        return nav
