...
class CommunityScreen(Screen):
    ...
    def _build_hot_users(self):
        ...
        for idx, name in enumerate(HOT_USER_NAMES):
            item = BoxLayout(orientation="vertical", spacing=dp(6),
                             size_hint=(1, 1))
    ...
    def _build_post_card(self, post):
        ...
        if avatar_path and os.path.exists(avatar_path):
            ...
            avatar = GrayPlaceholder(
                radius=1, size_hint=(None, None), size=(dp(48), dp(48)))
        ...
        # 阅读量
        view_row = BoxLayout(size_hint=(1, None), height=dp(22))
        view_count = STORE_DB.get_post_view_count(
            post.get("key", f"post:{post['id']}"))
        view_lbl = Label(
            text=f"阅读 {view_count}",
            font_size=sp(13), color=(0.55, 0.55, 0.55, 1),
            size_hint=(None, 1), width=dp(80),
            halign="left", valign="middle", **text_style(),
        )
        view_lbl.bind(size=view_lbl.setter("text_size"))
        view_row.add_widget(view_lbl)
        view_row.add_widget(Widget(size_hint=(1, 1)))
        card.add_widget(view_row)
        ...
        # 点赞图片路径
        like_off = os.path.join(IMAGE_DIR, "点赞（未点赞状态）.jpg")
        like_on = os.path.join(IMAGE_DIR, "点赞（点赞状态）.jpg")
        ...
        like_outer = FloatLayout(size_hint=(None, 1), width=dp(84))
        like_inner = BoxLayout(size_hint=(1, 1), spacing=dp(4))
        like_inner.add_widget(heart_img)
        like_inner.add_widget(like_count_lbl)
        like_outer.add_widget(like_inner)
        like_outer.bind(
            pos=lambda i, *_: setattr(like_inner, "pos", i.pos),
            size=lambda i, *_: setattr(like_inner, "size", i.size),
        )
        like_outer.add_widget(like_btn)
        footer.add_widget(like_outer)

        footer.add_widget(Widget(size_hint=(1, 1)))

        # 分享圆圈
        share_wrap = FloatLayout(size_hint=(None, 1), width=dp(44))
        share_circle = Label(
            size_hint=(None, None), size=(dp(34), dp(34)),
            pos_hint={"center_x": 0.5, "center_y": 0.5},
        )

        def _draw_share(lbl, *_):
            lbl.canvas.before.clear()
            with lbl.canvas.before:
                Color(0.35, 0.35, 0.85, 1)
                Line(
                    circle=(lbl.center_x, lbl.center_y, dp(15)),
                    width=dp(1.5),
                )

        share_circle.bind(pos=_draw_share, size=_draw_share)
        share_text = Label(
            text="分享",
            font_size=sp(11),
            color=(0.35, 0.35, 0.85, 1),
        ...
        share_text.bind(size=share_text.setter("text_size"))
        share_btn = Button(
            size_hint=(1, 1),
            background_normal="", background_down="",
            background_color=(0, 0, 0, 0),
        ...
        share_wrap.add_widget(share_circle)
        share_wrap.add_widget(share_text)
